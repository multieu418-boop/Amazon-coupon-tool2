...
def load_excel_template(file):
    ...
    ws = wb.active
    data = []
    headers = [cell.value for cell in ws[7]] # 第7行是 Header
...
def main():
    ...
    if l_file and e_file:
        ...
        if st.session_state.master_df is not None:
            ...
            df_filtered = st.session_state.master_df[mask]

            # 决策台展示
            st.subheader("🛠️ 修复决策台")
            
            # 定义样式：高折扣标红
            def color_high_discount(val):
                color = 'red' if isinstance(val, (int, float)) and val > discount_threshold else 'black'
                return f'color: {color}; font-weight: bold' if color == 'red' else ''

            # 使用 data_editor 并同步回 Session State
            ...
            # 关键：手动同步修改到总表（解决筛选后丢失的问题）
            if not updated_df.equals(df_filtered):
                for index, row in updated_df.iterrows():
                    st.session_state.master_df.loc[index, '决策'] = row['决策']
                    st.session_state.master_df.loc[index, '拟提报折扣'] = row['拟提报折扣']
            ...
            if st.button("🚀 导出全量原格式文件 (包含正确+已保留报错项)"):
                ...
                if final_keep.empty:
                    ...
                    out = io.BytesIO()
                    wb.save(out)
                    st.success(f"成功导出！共 {len(final_keep)} 个 ASIN。")
                    st.download_button("📥 点击下载最终修复文件", out.getvalue(), "Coupon_Final_Ready.xlsx")
