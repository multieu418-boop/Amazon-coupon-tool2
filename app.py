...
def main():
    ...
    with col1:
        l_file = st.file_uploader("1. 上传 All Listing 报告", type=['txt', 'xlsx', 'csv'], key="l_file")
    with col2:
        e_file = st.file_uploader("2. 上传带批注的报错模板", type=['xlsx'], key="e_file")
        ...
        if st.session_state.master_df is not None:
            ...
            if not edited_data.equals(df_filtered):
                for idx in edited_data.index:
                    st.session_state.master_df.at[idx, '决策'] = edited_data.at[idx, '决策']
                    st.session_state.master_df.at[idx, '拟提报折扣'] = edited_data.at[idx, '拟提报折扣']
                st.rerun()
            ...
            # 使用 container 包裹导出逻辑，防止其在筛选时消失
            export_container = st.container()
            with export_container:
                if st.button("🚀 生成并导出 Excel (全量合并版)", key="export_btn"):
                    e_file.seek(0)
                    ...
                    if final_keep.empty:
                        st.error("❌ 当前没有任何 ASIN 被标记为‘保留’，请检查决策台。")
                    else:
                        a_idx, d_idx = 1, 3
                        ...
                        st.download_button(
                            label="📥 点击下载 Final_Coupon.xlsx",
                            data=out_io.getvalue(),
                            file_name="Final_Coupon_Fixed.xlsx",
